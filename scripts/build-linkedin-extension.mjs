...
if (release) {
  ...
  if (process.platform === 'win32') {
    ...
    // Compress a closed list, never arbitrary leftovers from dist or developer files.
    const paths = [...releaseFiles, 'panel.js', 'panel.css', 'manifest.json'].map(file => quote(resolve(output, file))).join(',');
    execFileSync('powershell.exe', ['-NoProfile', '-Command', `Compress-Archive -Path ${paths} -DestinationPath ${quote(zip)} -Force`]);

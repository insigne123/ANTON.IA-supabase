...
ensure_env_secret FIREBASE_SCHEDULER_SECRET
...
printf '%s' "$FIREBASE_SCHEDULER_SECRET" | \
  firebase apphosting:secrets:set FIREBASE_SCHEDULER_SECRET -P "$TARGET_PROJECT" --data-file - -f

firebase apphosting:secrets:grantaccess \
  ENRICHMENT_SERVICE_SECRET,SERPER_API_KEY,AZURE_AD_CLIENT_SECRET,GOOGLE_CLIENT_SECRET,SUPABASE_SERVICE_ROLE_KEY,LEAD_RESEARCH_WORKER_SECRET,FIREBASE_SCHEDULER_SECRET,ANTONIA_TICK_SECRET,TRACKING_WEBHOOK_SECRET \
  -P "$TARGET_PROJECT" \
  -b "$BACKEND" \
  -l "$LOCATION"

...
TARGET_PROJECT="${TARGET_PROJECT:-${SOURCE_PROJECT}}"
...
ensure_env_secret TRACKING_TOKEN_SECRET
...
printf '%s' "$TRACKING_TOKEN_SECRET" | \
  firebase apphosting:secrets:set TRACKING_TOKEN_SECRET -P "$TARGET_PROJECT" --data-file - -f

firebase apphosting:secrets:grantaccess \
  APOLLO_API_KEY,AZURE_AD_CLIENT_SECRET,GOOGLE_CLIENT_SECRET,SUPABASE_SERVICE_ROLE_KEY,VANE_AUTH_HEADER_VALUE,LEAD_RESEARCH_WORKER_SECRET,TRACKING_TOKEN_SECRET,ANTONIA_TICK_SECRET,TRACKING_WEBHOOK_SECRET \
  -P "$TARGET_PROJECT" \
  -b "$BACKEND" \
  -l "$LOCATION"

...
/** Scoped preview of the staged messaging-context patch: exact values pinned
 * by the proposal target, shown against current context. Never applies anything. */
export async function GET(_req: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!proposal || proposal.action !== 'cowork.effect' || proposal.kind !== 'message_context_update') {
      return NextResponse.json({ error: 'No hay propuesta de contexto en este trabajo.' }, { status: 409, headers: privateHeaders });
    }
    const client = getSupabaseAdminClient();
    const row = await client.from('cowork_message_context_proposals')

...
import { countUniqueMeetingRequestContacts, countUniquePositiveReplyContacts, countUniqueReplyContacts } from '@/lib/antonia-reply-metrics';
...
async function computeMissionMetrics(admin: any, mission: any) {
  ...
  const [{ data: missionContacts }, { data: missionReplyRows }] = await Promise.all([
    admin
      .from('contacted_leads')
  ...
  const contactRows = (missionContacts || []) as any[];
  const replyRows = (missionReplyRows || []) as any[];

  metrics.contactsTotal = contactRows.length;
  metrics.opensTotal = contactRows.filter((row) => row.opened_at).length;
  metrics.repliesTotal = countUniqueReplyContacts(contactRows, replyRows);
  metrics.positiveRepliesTotal = countUniquePositiveReplyContacts(contactRows);
  metrics.meetingRequestsTotal = countUniqueMeetingRequestContacts(contactRows);
...
export async function PATCH(req: NextRequest, context: { params: Promise<{ missionId: string }> }) {
  try {
    ...
    const nextParams: any = {
      ...
      applyIcpFilter: Object.prototype.hasOwnProperty.call(updates, 'applyIcpFilter')
        ? Boolean(updates.applyIcpFilter)
        : currentParams.applyIcpFilter !== false,
    ...
    for (const t of (pendingTasks || [])) {
      const payload = { ...(t.payload || {}) };
        const commonPatch = {
          ...
          applyIcpFilter: nextParams.applyIcpFilter,

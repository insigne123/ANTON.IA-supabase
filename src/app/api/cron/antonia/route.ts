...
async function executeContact(task: any, supabase: any) {
    ...
    for (const lead of leadsToContact) { // [FIX #8] use capped list
        ...
        // Autopilot may qualify and schedule a lead, but it must never send a
        // browser-free generated message without an explicit review/approval.
        await createAntoniaException(supabase, {
            organizationId: task.organization_id,
            missionId: task.mission_id,
            taskId: task.id,
            ...
            category: 'approval_required',
            severity: 'low',
            title: 'Correo listo para revisión',
            description: `ANTONIA preparó el siguiente paso para ${leadContext.email}, pero requiere revisión y aprobación antes de enviar.`,
            dedupeKey: `approval_required_${task.mission_id}_${leadContext.id || leadContext.email}`,
            payload: { lead: leadContext, campaignName, scheduledFor: schedule.scheduledFor },
        });
        await syncLeadAutopilotToCrm(supabase, {
            organizationId: task.organization_id,
            leadId: leadContext.id || null,
            stage: 'qualified',
            notes: 'Contacto preparado; requiere revisión manual antes de enviar.',
            nextAction: 'Revisar y aprobar correo',
            nextActionType: 'approval_required',
        ...
        try {
            ...
            if (shouldAppendContactedLead(response.status, responseData)) {
                const dispatchId = String(resData.dispatchId || '').trim();
                const { data: projectedContact, error: projectedContactError } = dispatchId
                    ? await supabase
                        .from('contacted_leads')
                        .select('id, mission_id')
                        .eq('organization_id', task.organization_id)
                        .eq('user_id', task.payload.userId)
                        .contains('data', { dispatchId })
                        .maybeSingle()
                    : { data: null, error: null };

                if (projectedContactError) {
                    console.warn('[Contact] Could not verify canonical dispatch history', projectedContactError);
                } else if (projectedContact) {
                    projectedContactedCount += 1;
                    contactResult.contactedId = projectedContact.id;
                    if (task.mission_id && projectedContact.mission_id !== task.mission_id) {
                        const { error: projectionUpdateError } = await supabase
                            .from('contacted_leads')
                            .update({ mission_id: task.mission_id, last_update_at: new Date().toISOString() })
                            .eq('id', projectedContact.id)
                            .eq('organization_id', task.organization_id)
                            .eq('user_id', task.payload.userId);
                        if (projectionUpdateError) {
                            console.warn('[Contact] Could not attach mission to canonical dispatch history', projectionUpdateError);
                        }
                    }
                } else {
                    contactedLeads.push({
            ...
            const recordsForCampaign = campaignSentRecords ?? {};
            if (campaign?.id && leadContext?.id) {
                campaignSentRecords = recordsForCampaign;
                const leadKey = String(leadContext.id);
                const existingSentRecord = recordsForCampaign[leadKey];
                if (!isReplayed || !existingSentRecord || Number(existingSentRecord.lastStepIdx) < 0) {
                    recordsForCampaign[leadKey] = { lastStepIdx: 0, lastSentAt: sentAt };
                    campaignDirty = true;
                }
            }
    ...
    const contactedCount = contactedLeads.length + projectedContactedCount;
    if (contactedCount > 0) {
        await incrementUsage(supabase, task.organization_id, 'contact', contactedCount);
    }
    ...
    console.log(`[Contact] Successfully sent ${contactedCount} emails. Replayed: ${replayedCount}. Failed: ${errors.length}`);
...
const LEGACY_ANTONIA_CRON_HEADERS = {
    'Cache-Control': 'no-store',
    'Deprecation': 'true',
    'Link': '</docs/deployment.md#scheduler-de-produccion>; rel="deprecation"',
    'X-Scheduler-Owner': 'firebase-functions',
};

function legacyAntoniaCronResponse() {
    return NextResponse.json({
        ok: false,
        code: 'LEGACY_ANTONIA_CRON_DEPRECATED',
        deprecated: true,
        schedulerOwner: 'firebase-functions',
        replacement: 'Firebase scheduled function antoniaTick',
        message: 'El scheduler de ANTON.IA se ejecuta exclusivamente desde Firebase Functions.',
    }, {
        status: 410,
        headers: LEGACY_ANTONIA_CRON_HEADERS,
    });
}
...
export async function GET(request: Request) {
    ...
    return legacyAntoniaCronResponse();
}

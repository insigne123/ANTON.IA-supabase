...
function isMissionIcpFilterEnabled(params?: any) {
    return params?.applyIcpFilter !== false;
}
...
export default function AntoniaPage() {
    ...
    useEffect(() => {
        async function checkConnectionStatus() {
            ...
            try {
                ...
                setGoogleConnected(Boolean(connections?.google));
                setOutlookConnected(Boolean(connections?.outlook));
            } catch (error) {
    ...
    const handleToggleMissionIcpFilter = async (mission: AntoniaMission) => {
        const nextApplyIcpFilter = !isMissionIcpFilterEnabled(mission.params);

        try {
            const res = await fetch(`/api/antonia/missions/${mission.id}/intelligence`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ updates: { applyIcpFilter: nextApplyIcpFilter } }),
            });

            const data = await res.json().catch(() => ({}));
            if (!res.ok) throw new Error(data?.error || 'No se pudo actualizar el filtro ICP');

            handleMissionUpdatedFromTuner(data.mission);
            toast({
                title: nextApplyIcpFilter ? 'Filtro ICP activado' : 'Filtro ICP desactivado',
                description: nextApplyIcpFilter
                    ? 'La misión volverá a descartar leads fuera del perfil ideal.'
                    : 'La misión guardará los leads del search sin el filtro extra de ICP.',
            });
        } catch (error: any) {
            console.error('[ANTONIA] Error toggling mission ICP filter:', error);
            toast({
                variant: 'destructive',
                title: 'Error',
                description: error?.message || 'No se pudo actualizar el filtro ICP.',
            });
        }
    };
                                    ...
                                    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                                        ...
                                        <div className="space-y-3 rounded-2xl border border-border/60 bg-background/60 p-4">
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                {wizardData.applyIcpFilter
                                                    ? 'Activo: ANTONIA descartará leads fuera del perfil ideal antes de guardarlos.'
                                                    : 'Desactivado: ANTONIA guardará todos los leads que cumplan el search inicial.'}
                                            </p>
                                        </div>
                                ...
                                {step === 3 && (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                        ...
                                        <div className="bg-secondary/30 p-4 rounded-lg border space-y-3">
                                            ...
                                            <div className="flex justify-between">
                                                <span className="text-muted-foreground">Filtro ICP:</span>
                                                <span className="font-medium">{wizardData.applyIcpFilter ? 'Activado' : 'Desactivado'}</span>
                                            </div>
                                            <div className="flex justify-between">
                                ...
                                <Card key={mission.id} className="relative overflow-hidden group hover:border-primary/50 transition-colors">
                                    ...
                                    <CardContent className="relative z-10">
                                        ...
                                        <div className="mb-4 p-3 bg-secondary/20 rounded-lg border text-xs space-y-1">
                                            ...
                                            <div className="flex justify-between">
                                                <span className="text-muted-foreground">Filtro ICP:</span>
                                                <span className="font-medium">{isMissionIcpFilterEnabled(mission.params) ? 'Activado' : 'Desactivado'}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 grid grid-cols-2 gap-2">
                                            ...
                                            <Button
                                                variant={isMissionIcpFilterEnabled(mission.params) ? 'outline' : 'secondary'}
                                                size="sm"
                                                className="w-full col-span-2"
                                                onClick={() => handleToggleMissionIcpFilter(mission)}
                                            >
                                                {isMissionIcpFilterEnabled(mission.params) ? 'Desactivar filtro ICP' : 'Activar filtro ICP'}
                                            </Button>
                                            <Button
                                                variant="secondary"
                                                size="sm"

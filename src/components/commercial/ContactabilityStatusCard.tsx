...
export function ContactabilityStatusCard({ email, result: providedResult, loading: providedLoading, error: providedError, compact, onRetry }: Props) {
  ...
  if (loading) {
    return (
      <div className={cn('rounded-xl border bg-card p-4', compact && 'p-3')}>
  ...
  if (error) {
    return (
      <div className={cn('rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-950 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100', compact && 'p-3')}>
  ...
  if (!result) {
    return (
      <div className={cn('rounded-xl border border-dashed p-4 text-sm text-muted-foreground', compact && 'p-3')}>
  ...
  return (
    <div className={cn('rounded-xl border p-4', statusClasses(result.status), compact && 'p-3')}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        {email && <Badge variant="outline" className="w-fit border-current/20 bg-background/40 text-current">Privacy check</Badge>}
      </div>
    </div>
  );
}

...
export function BulkCampaignWorkspace() {
  ...
  const [audienceProposal, setAudienceProposal] = useState<{ criteria: CampaignInput['criteria']; explanation: string; unresolved: string[] } | null>(null);
  ...
  function changeMessage(patch: Partial<CampaignMessage>) { change({ messages: definition.messages.map((value, index) => index === messageIndex ? { ...value, ...patch } : value) }); setProposal(null); }
  ...
  async function searchAudience(page: number) {
    ...
    setPeople(result.people); setAudienceTotal(result.total); setAudiencePage(result.page); setSearched(true);
  }
        ...
        {!editing ? <>
          ...
          <section className="space-y-5 rounded-2xl border bg-card p-5 sm:p-7" aria-busy={busy}>
            {step === 0 && <>
              ...
              <div className="space-y-2"><Label htmlFor="audience-description">Describe tu audiencia ideal</Label><Textarea id="audience-description" disabled={busy} value={definition.description} onChange={event => change({ description: event.target.value })} placeholder="Responsables de operaciones en Chile a los que todavía no contactamos" /><Button variant="outline" disabled={busy || definition.description.trim().length < 10} onClick={() => void run(async () => setAudienceProposal(await request('/assist', { mode: 'audience', description: definition.description, criteria: definition.criteria })))}>Interpretar con IA</Button></div>
              {audienceProposal && <div className="space-y-3 rounded-xl border bg-muted/40 p-4"><p>{audienceProposal.explanation}</p>{audienceProposal.unresolved.length > 0 && <p className="text-sm">Falta precisar: {audienceProposal.unresolved.join('; ')}. Estos requisitos no se aplicarán a la búsqueda.</p>}<Button variant="outline" disabled={busy} onClick={() => { change({ criteria: AudienceCriteriaSchema.parse(audienceProposal.criteria) }); setAudienceProposal(null); setSearched(false); }}>Usar estos criterios y revisarlos</Button></div>}
              ...
              <div className="flex flex-wrap gap-2"><Input className="max-w-72" disabled={busy} value={audienceQuery} onChange={event => setAudienceQuery(event.target.value)} placeholder="Buscar por nombre, empresa o correo" aria-label="Buscar en resultados" /><Button disabled={busy} onClick={() => void run(() => searchAudience(0))}>{busy ? 'Buscando…' : 'Buscar en mis leads'}</Button></div>
              {searched && <><p className="text-sm" role="status">{audienceTotal} coincidencias · {definition.emails.length} seleccionados (máximo 100)</p>{!people.length && <p className="text-sm text-muted-foreground">No encontramos coincidencias. Prueba con menos criterios o revisa los datos de tus leads.</p>}<div className="max-h-80 space-y-1 overflow-y-auto">{people.map(person => <label key={person.email} className="flex items-start gap-3 rounded-lg p-3 hover:bg-muted/50"><input className="mt-1" type="checkbox" disabled={busy || Boolean(person.blockedReason) || (!definition.emails.includes(person.email) && definition.emails.length >= 100)} checked={definition.emails.includes(person.email)} onChange={event => change({ emails: event.target.checked ? [...definition.emails, person.email] : definition.emails.filter(email => email !== person.email) })} /><span className="min-w-0 text-sm"><span className="block font-medium">{person.name || person.email} · {person.company}</span><span className="block break-all text-muted-foreground">{person.email}</span><span className="text-muted-foreground">{person.blockedReason || person.reasons.join(' · ')}</span></span></label>)}</div>{audienceTotal > 25 && <div className="flex items-center justify-between gap-2 text-sm"><Button variant="outline" size="sm" disabled={busy || audiencePage === 0} onClick={() => void run(() => searchAudience(audiencePage - 1))}>Anterior</Button><span>Página {audiencePage + 1} de {Math.max(1, Math.ceil(audienceTotal / 25))}</span><Button variant="outline" size="sm" disabled={busy || (audiencePage + 1) * 25 >= audienceTotal} onClick={() => void run(() => searchAudience(audiencePage + 1))}>Siguiente</Button></div>}</>}
              {definition.emails.length > 0 && <details><summary className="cursor-pointer text-sm">Revisar selección guardada ({definition.emails.length})</summary><p className="my-2 text-sm text-muted-foreground">Los cambios de filtros no eliminan tu selección. Al guardar verificaremos que todos sigan cumpliendo los criterios.</p>{definition.emails.map(email => <div key={email} className="flex items-center justify-between gap-2 text-sm"><span className="break-all">{email}</span><Button variant="ghost" size="sm" disabled={busy} onClick={() => change({ emails: definition.emails.filter(value => value !== email) })}>Quitar</Button></div>)}</details>}
            ...
            {step === 1 && <>
              <h2 className="text-xl font-semibold">{reviseMode ? 'Edita los mensajes pendientes' : 'Escribe el mensaje que quieres compartir'}</h2>
              {reviseMode && <p className="text-sm text-muted-foreground">Los mensajes ya enviados o en curso están bloqueados. Al guardar, los pendientes vuelven a revisión y necesitarán una nueva aprobación.</p>}
              <div className="space-y-2"><Label htmlFor="objective">¿Qué quieres conseguir?</Label><Textarea id="objective" disabled={busy} value={definition.objective} onChange={event => change({ objective: event.target.value })} placeholder="Presentar nuestro servicio y proponer una reunión breve" /></div>
              <div className="flex flex-wrap gap-2">{definition.messages.map((_, index) => <Button key={index} disabled={busy || (reviseMode && reviseLocked(index) && messageIndex !== index)} variant={messageIndex === index ? 'secondary' : 'ghost'} onClick={() => { setMessageIndex(index); setProposal(null); }}>{index === 0 ? 'Primer correo' : `Seguimiento ${index}`}{reviseMode && reviseLocked(index) ? ' · Bloqueado' : ''}</Button>)}{definition.messages.length < 5 && !reviseMode && <Button variant="outline" disabled={busy} onClick={() => { change({ messages: [...definition.messages, { subject: '', body: '', delayDays: 3 }] }); setMessageIndex(definition.messages.length); setProposal(null); }}>Añadir seguimiento</Button>}</div>
              {messageIndex > 0 && <div className="space-y-2"><Label htmlFor="delay">Días después del envío anterior</Label><Input id="delay" type="number" min={1} max={90} disabled={busy || reviseLocked(messageIndex)} value={message.delayDays} onChange={event => changeMessage({ delayDays: Number(event.target.value) })} />{!reviseMode && <Button variant="ghost" disabled={busy} onClick={() => { change({ messages: definition.messages.filter((_, index) => index !== messageIndex) }); setMessageIndex(0); }}>Quitar este seguimiento</Button>}</div>}
              <div className="space-y-2"><Label htmlFor="subject">Asunto</Label><Input id="subject" disabled={busy || reviseLocked(messageIndex)} value={message.subject} onChange={event => changeMessage({ subject: event.target.value })} /></div>
              <div className="space-y-2"><Label htmlFor="body">Correo</Label><Textarea id="body" className="min-h-64" disabled={busy || reviseLocked(messageIndex)} value={message.body} onChange={event => changeMessage({ body: event.target.value })} /><p className="text-sm text-muted-foreground">Puedes usar {'{{nombre}}'}, {'{{empresa}}'} y {'{{cargo}}'} con datos reales. Incluye tu firma en el mensaje.</p></div>
              <div className="space-y-2"><Label htmlFor="instruction">Pide ayuda a la IA</Label><Input id="instruction" disabled={busy || reviseLocked(messageIndex)} value={instruction} onChange={event => setInstruction(event.target.value)} placeholder="Escribe un correo breve o mejora el cierre" /><Button variant="outline" disabled={busy || reviseLocked(messageIndex) || instruction.trim().length < 5} onClick={() => void run(async () => setProposal((await request('/assist', { mode: 'message', instruction, objective: definition.objective, current: message, relationship: definition.criteria.relationship })).proposal))}>Proponer cambios</Button></div>
              {proposal && <div className="space-y-3 rounded-xl border bg-muted/40 p-4"><h3 className="font-medium">Propuesta de IA</h3><p className="font-medium">{proposal.subject}</p><p className="whitespace-pre-wrap text-sm">{proposal.body}</p><div className="flex gap-2"><Button disabled={busy} variant="secondary" onClick={() => changeMessage(proposal)}>Aplicar propuesta</Button><Button variant="ghost" disabled={busy} onClick={() => setProposal(null)}>Descartar</Button></div></div>}
              {!reviseMode && <div className="space-y-2"><Label htmlFor="provider">Enviar desde</Label><select id="provider" disabled={busy} className="h-11 w-full rounded-md border bg-background px-3 focus-visible:outline-ring" value={definition.provider} onChange={event => change({ provider: event.target.value as 'google' | 'outlook' })}><option value="google">Mi cuenta de Gmail conectada</option><option value="outlook">Mi cuenta de Outlook conectada</option></select></div>}
              ...
              <div className="flex justify-between gap-3"><Button variant="ghost" disabled={busy} onClick={() => { if (reviseMode) { setReviseMode(false); setStep(2); } else setStep(0); }}>{reviseMode ? 'Cancelar edición' : 'Atrás'}</Button><Button disabled={busy} onClick={() => void run(() => save())}>{busy ? 'Preparando…' : reviseMode ? 'Guardar pendientes y volver a revisión' : 'Guardar y revisar correos'}</Button></div>
            </>}
            {step === 2 && campaign && <>

...
export const antoniaService = {
    ...
    getConfig: async (organizationId: string): Promise<AntoniaConfig | null> => {
        ...
        return {
            ...
            replyAutopilotEnabled: data.reply_autopilot_enabled,
            replyAutopilotMode: data.reply_autopilot_mode,
            replyApprovalMode: data.reply_approval_mode,
            replyMaxAutoTurns: data.reply_max_auto_turns,
            autoSendBookingReplies: data.auto_send_booking_replies,
            allowReplyAttachments: data.allow_reply_attachments,
    ...
    upsertConfig: async (config: Partial<AntoniaConfig> & { organizationId: string }) => {
        ...
        const dbPayload = {
            ...
            daily_enrich_limit: config.dailyEnrichLimit ?? 100,
            daily_investigate_limit: config.dailyInvestigateLimit ?? 100,
            ...
            reply_autopilot_enabled: config.replyAutopilotEnabled ?? false,
            reply_autopilot_mode: config.replyAutopilotMode ?? 'draft_only',
            reply_approval_mode: config.replyApprovalMode ?? 'high_risk_only',
            reply_max_auto_turns: config.replyMaxAutoTurns ?? 2,
            auto_send_booking_replies: config.autoSendBookingReplies ?? false,
            allow_reply_attachments: config.allowReplyAttachments ?? false,

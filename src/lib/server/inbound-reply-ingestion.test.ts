...
test('shared inbound RPC preserves stable provider, message, recipient, and contact context', async () => {
  ...
  assert.ok(calls.every((call) => call.name === 'ingest_inbound_reply_v1'));
  assert.deepEqual(
    calls.map((call) => [
...
test('inbound ingestion RPC is service-role-only and atomically owns all durable reply writes', () => {
  ...
  assert.match(migration, /revoke all on function public\.ingest_inbound_reply_v1[\s\S]*from public/);
  assert.match(migration, /grant execute on function public\.ingest_inbound_reply_v1[\s\S]*to service_role/);
  assert.match(body, /insert into public\.lead_responses/);
  assert.match(body, /insert into public\.email_events/);
  assert.match(body, /update public\.contacted_leads/);
  assert.match(body, /engagement_score = coalesce\(engagement_score, 0\) \+ case when v_is_failure then 0 else 10 end/);
  assert.match(body, /reply_intent = v_intent/);
...
test('webhook and provider sync gate unsubscribe and downstream effects on a new RPC insert', () => {
  for (const [name, source] of [['tracking webhook', webhook], ['reply sync', replySync]] as const) {
    const rpc = source.indexOf('await ingestInboundReply(');
    const insertedGuard = source.indexOf('if (!ingestion.inserted)', rpc);
    const suppression = source.indexOf('if (shouldGloballySuppressReply(', rpc);

    assert.ok(rpc > -1, `${name} must call the shared RPC`);
    assert.ok(insertedGuard > rpc, `${name} must inspect inserted`);
    assert.ok(suppression > insertedGuard, `${name} suppression must follow inserted=true`);
  }

...
const FULLENRICH_ENRICHMENT_ROUTE = '/api/opportunities/enrich-apollo';
...
async function enrichLeadBatch(input: Record<string, unknown>, context: SupliaToolContext) {
  ...
  const { url, internalSecret } = resolveFullEnrichEnrichmentRoute();
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      leads: leads.map((lead) => ({
        ...
        sourceProviderId: asText(lead.sourceProviderId || lead.source_provider_id) || undefined,
        clientRef: asText(lead.id) || undefined,
      })),
    }),
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(String(payload?.error || `FULLENRICH_ENRICHMENT_${response.status}`));
...
function resolveFullEnrichEnrichmentRoute() {
  ...
  try {
    return { url: new URL(FULLENRICH_ENRICHMENT_ROUTE, baseUrl).toString(), internalSecret };
  } catch {
...
const SUPLIA_TOOLS: Record<string, SupliaToolDefinition> = {
  ...
  'prospecting.search_companies': {
    ...
    description: 'Busca empresas con FullEnrich. Consume creditos externos y requiere aprobacion humana.',
  ...
  'prospecting.search_people': {
    ...
    description: 'Busca personas decisoras con FullEnrich por empresa, dominio, titulo y ubicacion. Consume creditos externos y requiere aprobacion humana.',

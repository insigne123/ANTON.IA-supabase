...
test('manual-review deletion is reported honestly by server, API, and UI', () => {
  ...
  assert.match(privacySubjectActionsRoute, /success: false,\s+outcome: 'manual_review'/);
  assert.match(privacySubjectActionsRoute, /\}, \{ status: 409 \}\);/);
  assert.ok(
    runSubjectAction.indexOf("data?.result?.outcome === 'manual_review'")

...
const persistedDispatchId = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
...
async function finalizeConfirmedSentHistory(
  result: OutboundDispatchResult,
  dependencies?: { finalizeSentHistory?: OutboundSentHistoryFinalizer; client?: SupabaseClientLike },
): Promise<OutboundDispatchResult> {
  if (result.status !== 'sent') return result;

  const finalize = dependencies?.finalizeSentHistory
    ?? ((dispatch: OutboundDispatch) => finalizeSentOutboundDispatchHistory(dispatch.id, dependencies?.client));

  try {
    await finalize(result.dispatch);
  } catch (error) {
    // Provider confirmation remains durable; the reconciliation job retries pending history repair.
    console.error('[outbound-dispatch] failed to finalize confirmed sent history', {
...
async function finalizeCampaignDelivery(
  result: OutboundDispatchResult,
  dependencies?: { finalizeCampaignDelivery?: OutboundCampaignDeliveryFinalizer; client?: SupabaseClientLike },
): Promise<OutboundDispatchResult> {
  if (
    !isCampaignDispatchKey(result.dispatch.idempotencyKey)
    || (!dependencies?.finalizeCampaignDelivery && !persistedDispatchId.test(result.dispatch.id))
  ) return result;

  const finalize = dependencies?.finalizeCampaignDelivery
    ?? ((dispatch: OutboundDispatch) => finalizeCampaignDeliveryOutcome(dispatch.id, dependencies?.client));

  try {
    await finalize(result.dispatch);
  } catch (error) {
    // The dispatch remains authoritative; a same-key replay retries campaign projection without resending.
    console.error('[outbound-dispatch] failed to finalize campaign delivery', {
      dispatchId: result.dispatch.id,
      error,
    });
  }
...
export async function reconcileUnknownOutboundDispatch(
  ...
  await finalizeCampaignDelivery({ status: updated.status, dispatch: updated, replayed: false }, {
    client,
    finalizeCampaignDelivery: dependencies?.finalizeCampaignDelivery,
  });
  if (updated.status === 'failed') {
    await releaseQuota(createSupabaseOutboundDispatchRepository(client), updated.id);
  }
  if (updated.status === 'sent') {
    await finalizeConfirmedSentHistory({ status: 'sent', dispatch: updated, replayed: false }, {
      client,
...
export async function dispatchOutboundMessage(
  ...
  try {
    ...
    return finish({ status: unknown.status, dispatch: unknown, replayed: false });
  }
  ...
  try {
    ...
    return finish({ status: sent.status, dispatch: sent, replayed: false });
  } catch (error) {
    const unknown = await recordUnknown(repository, dispatch, {
      code: 'persistence_error',

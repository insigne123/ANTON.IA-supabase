...
export async function repairReconciledOutboundDispatchHistories(input?: {
  ...
  for (const row of data || []) {
    ...
    try {
      ...
      if (!repairResult.repaired && !repairResult.finalized) {
        throw new Error(`History repair skipped: ${repairResult.reason || 'unknown_reason'}`);
      }
      if (!repairResult.finalized) {
        await recordHistoryRepairAttempt(admin, dispatch, { status: 'complete', attemptedAt });
      }
      results.push({
        id: dispatch.id,

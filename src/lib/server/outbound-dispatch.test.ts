...
function testDependencies(
  ...
  overrides: Partial<OutboundDispatchDependencies> = {},
) {
  return {
    repository,
    now,
    finalizeSentHistory: async () => undefined,
    ...overrides,
  } satisfies OutboundDispatchDependencies;
}
...
test('a duplicate key makes one provider call and replays the sent dispatch', async () => {
  ...
  const first = await dispatchOutboundMessage({ draft, metadata, provider: provider.provider }, testDependencies(repository));
  const duplicate = await dispatchOutboundMessage({ draft, metadata, provider: provider.provider }, testDependencies(repository));
...
test('confirmed and replayed sent email dispatches both invoke history finalization', async () => {
  ...
  const provider = countingProvider();
  const draft = readyDraft();
  const metadata = metadataFor(draft, 'send:history-projection');
  const finalized: Array<{ id: string; replayed: boolean }> = [];

  const dependencies = {
    repository,
    now,
    async finalizeSentHistory(dispatch: OutboundDispatch) {
      finalized.push({ id: dispatch.id, replayed: finalized.length > 0 });
    },
  };
  ...
  assert.deepEqual(finalized, [
    { id: first.dispatch.id, replayed: false },
    { id: first.dispatch.id, replayed: true },
  ]);
});

test('campaign dispatches project pending, sending, confirmed, and replayed outcomes without another provider call', async () => {
  const repository = new MemoryDispatchRepository();
  const provider = countingProvider();
  const draft = readyDraft();
  const metadata = metadataFor(
    draft,
    'campaign:50000000-0000-4000-8000-000000000001:contact-1:step:0',
  );
  const deliveryStates: OutboundDispatchStatus[] = [];
  const dependencies = testDependencies(repository, {
    async finalizeCampaignDelivery(dispatch) {
      deliveryStates.push(dispatch.status);
    },
  });
...
test('history finalization failure preserves the confirmed sent result for retry', async () => {
  ...
  try {
    const result = await dispatchOutboundMessage({
      draft: readyDraft(),
      metadata: metadataFor(readyDraft(), 'send:history-projection-retry'),
      provider: provider.provider,
    }, {
      repository,
      now,
      async finalizeSentHistory() {
        throw new Error('history RPC unavailable');
      },
    });
    ...
    assert.match(String(reports[0][0]), /failed to finalize confirmed sent history/);
  } finally {
...
test('concurrent duplicate observes sending and cannot invoke the provider', async () => {
  ...
  const firstPromise = dispatchOutboundMessage({ draft, metadata, provider }, testDependencies(repository));
...
test('the same idempotency key with changed content is a conflict', async () => {
  ...
  await assert.rejects(() => dispatchOutboundMessage({
    ...
    provider: provider.provider,
  }, testDependencies(repository)), OutboundDispatchConflictError);
  assert.equal(provider.calls(), 1);
});
...
test('confirmed provider rejection is failed and is never resent', async () => {
  ...
  const first = await dispatchOutboundMessage({ draft, metadata, provider: provider.provider }, testDependencies(repository));
  const replay = await dispatchOutboundMessage({ draft, metadata, provider: provider.provider }, testDependencies(repository));
...
test('the same key can send after a quota-deferred day resets', async () => {
  ...
  const first = await dispatchOutboundMessage({ draft, metadata, provider }, testDependencies(repository, { now: () => currentTime }));
  currentTime = '2026-08-14T00:01:00.000Z';
  const retry = await dispatchOutboundMessage({ draft, metadata, provider }, testDependencies(repository, { now: () => currentTime }));
...
test('quota RPC failure before provider invocation defers, releases, and permits a same-key retry', async () => {
  ...
  const retry = await dispatchOutboundMessage({ draft, metadata, provider }, testDependencies(repository));
...
test('concurrent retries of a deferred dispatch make one later provider call', async () => {
  ...
  await dispatchOutboundMessage({ draft, metadata, provider: deferredProvider.provider }, testDependencies(repository));
  ...
  const firstRetryPromise = dispatchOutboundMessage({ draft, metadata, provider: retryProvider }, testDependencies(repository));
  await Promise.resolve();
  await Promise.resolve();
  const concurrentRetry = await dispatchOutboundMessage({ draft, metadata, provider: retryProvider }, testDependencies(repository));
  releaseProvider();
...
test('network ambiguity becomes unknown and is never resent', async () => {
  ...
  const first = await dispatchOutboundMessage({ draft, metadata, provider }, testDependencies(repository));
  const replay = await dispatchOutboundMessage({ draft, metadata, provider }, testDependencies(repository));
...
test('reconciliation promotes an unknown dispatch only with provider evidence', async () => {
  ...
  const campaignFinalized: string[] = [];
  ...
  const result = await reconcileUnknownOutboundDispatch(dispatch, {
    ...
    client,
    now,
    async finalizeSentHistory(sent) {
      finalized.push(sent.id);
    },
    async finalizeCampaignDelivery(dispatch) {
      campaignFinalized.push(dispatch.id);
    },
  });
  ...
  assert.deepEqual(campaignFinalized, [dispatch.id]);
  assert.deepEqual(finalized, [dispatch.id]);
});

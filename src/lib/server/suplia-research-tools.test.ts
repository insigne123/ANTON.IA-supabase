...
test('builds safe SerpAPI research queries', () => {
  assert.equal(
...
test('parses SerpAPI organic results without leaking request metadata', () => {
  const parsed = supliaResearchTestInternals.parseSerpOrganicResults({
    organic_results: [{ title: 'Acme launches', link: 'https://news.example/acme', snippet: 'Growth signal', displayed_link: 'news.example' }],
    related_questions: [{ question: 'What does Acme do?' }],
    search_information: { total_results: 10, time_taken_displayed: 0.2 },
  });

  assert.deepEqual(parsed.items, [{ title: 'Acme launches', link: 'https://news.example/acme', snippet: 'Growth signal', source: 'news.example', date: null }]);
  assert.deepEqual(parsed.relatedQuestions, ['What does Acme do?']);

...
test('organization search forwards an optional company name filter to Apollo', async () => {
  const requests: Array<{ url: URL; init?: RequestInit }> = [];
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (input, init) => {
    requests.push({ url: new URL(String(input)), init });
  ...
  try {
    ...
    assert.equal(requests[0]?.url.pathname, '/api/v1/mixed_companies/search');
    const params = requests[0]?.url.searchParams;
    assert.equal(params?.get('q_organization_name'), 'Adecco');
    assert.deepEqual(params?.getAll('q_organization_keyword_tags[]'), ['outsourcing']);
    assert.deepEqual(params?.getAll('organization_locations[]'), ['chile']);
  } finally {
    globalThis.fetch = originalFetch;
  }

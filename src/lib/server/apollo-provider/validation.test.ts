...
import {
  ...
  validateOrganizationSearchInput,
} from './validation';
...
test('organization search accepts a company name as the only filter', () => {
  ...
  if (!parsed.ok) return;
  assert.equal(parsed.value.companyName, 'Adecco');
  const orgInput = validateOrganizationSearchInput({ company_name: 'Adecco' }, config);
  assert.equal(orgInput.ok, true);

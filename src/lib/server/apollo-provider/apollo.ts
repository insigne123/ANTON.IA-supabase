...
// Mirrors the production gateway (Backend-leads): industries map to Apollo
// organization_industry_tag_ids[], with unmatched values as keyword fallbacks.
...
export async function executeApolloOrganizationSearch(input: OrganizationSearchInput, apiKey: string, config: GatewayConfig) {
  ...
  if (companyName) query.set('q_organization_name', companyName);

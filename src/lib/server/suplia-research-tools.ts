...
function needFirstEnv(names: string[]) {
  for (const name of names) {
    const value = String(process.env[name] || '').trim();
    if (value) return value;
  }
  throw new Error(`Falta una variable de entorno para esta tool de research: ${names.join(' o ')}.`);
}
...
function parseSerpOrganicResults(payload: any) {
  const organic = Array.isArray(payload?.organic_results) ? payload.organic_results : [];
  const news = Array.isArray(payload?.news_results) ? payload.news_results : [];
  const items = (news.length ? news : organic).slice(0, 10).map((item: any) => ({
    title: item?.title || null,
    link: item?.link || item?.source?.link || null,
    snippet: item?.snippet || item?.description || null,
    source: item?.source?.name || item?.source || item?.displayed_link || null,
    date: item?.date || null,
  }));

  return {
    items,
    answerBox: payload?.answer_box ? {
      title: payload.answer_box?.title || null,
      answer: payload.answer_box?.answer || payload.answer_box?.snippet || null,
      link: payload.answer_box?.link || null,
    } : null,
    relatedQuestions: Array.isArray(payload?.related_questions)
      ? payload.related_questions.slice(0, 5).map((item: any) => item?.question).filter(Boolean)
      : [],
    searchInformation: payload?.search_information ? {
      totalResults: payload.search_information?.total_results || null,
      timeTakenDisplayed: payload.search_information?.time_taken_displayed || null,
    } : null,
  };
}
...
function buildSerpQuery(kind: string, input: Record<string, unknown>) {
  ...
  if (kind === 'serp_company_news') return `${base} noticias novedades crecimiento expansion`;
  if (kind === 'serp_competitors') return `${base} competidores alternativas mercado`;
  if (kind === 'serp_jobs_signals') return `${base} hiring jobs careers contratando ${location}`.trim();
...
async function researchSerp(input: Record<string, unknown>, context: SupliaToolContext, kind: 'serp_company_news' | 'serp_competitors' | 'serp_jobs_signals' | 'brand_mentions') {
  ...
  const cacheKey = `${kind}:${query}:${asText(input.language) || 'es'}:${asText(input.countryCode) || 'cl'}:${String(clampInt(input.limit, 10, 1, 10))}`;
  const cached = await getPersistentResearchCache(context, 'serpapi', cacheKey, input.cache !== false);
  if (cached) return cached;

  const apiKey = needFirstEnv(['SERPAPI_API_KEY', 'SERP_API_KEY', 'SERPAPI_KEY']);
  const params = new URLSearchParams({
    engine: 'google',
    q: query,
    hl: asText(input.language) || 'es',
    gl: asText(input.countryCode) || 'cl',
    num: String(clampInt(input.limit, 10, 1, 10)),
    api_key: apiKey,
  });
  if (kind === 'serp_company_news') params.set('tbm', 'nws');

  const result = await fetchJsonWithTimeout(`https://serpapi.com/search.json?${params.toString()}`, `serpapi_${kind}`);
  if (!result.ok) throw new Error(result.warning);
  const parsed = parseSerpOrganicResults(result.data);
  const output = {
    status: 'completed',
    source: 'serpapi',
    provider: 'serpapi',
    ...
    fetchedAt: new Date().toISOString(),
    ...parsed,
    estimatedCreditUse: { provider: 'serpapi', searches: 1 },
    note: 'Busqueda externa con SerpAPI. Consume creditos y fue ejecutada solo tras aprobacion.',
  };
  await setPersistentResearchCache(context, 'serpapi', cacheKey, output);
  return output;
}

...
import { RESEARCH_SEQUENCE_STEPS } from '@/lib/outreach-sequence-brief';
...
export async function researchSequenceView(job: any, dependencies: Pick<SequenceWorkerDependencies, 'getDraft' | 'queryPlan'> = {}) {
  ...
  const plan = initial ? await (dependencies.queryPlan || (await import('./campaigns-v2/plan')).queryFirstContactPlan)({ ...access, draftId: initial.draftId }) : null;
  const summaries = [initial ? { draftId: initial.draftId, versionId: initial.versionId, subject: initial.content.subject || '', body: initial.content.text || initial.content.html || '' } : null, ...RESEARCH_SEQUENCE_STEPS.map((_, index) => plan?.steps[index]?.draft || null)];
  ...
  return ResearchSequenceViewSchema.parse({
    ...
    slots: ['Contacto inicial', ...RESEARCH_SEQUENCE_STEPS.map((step) => step.name)].map((name, index) => ({
      index, name,
...
export async function processResearchSequenceQueue(input: { jobId?: string } = {}, dependencies: SequenceWorkerDependencies = {}) {
  ...
  try {
    ...
    if (!job.prepared_context) {
      const prepared = await (dependencies.prepareBrief || prepareNativeSequenceBrief)({ ...access, snapshotId: request.researchSnapshotId, styleProfileId: request.styleProfileId });
      ...
      await save({ ...queued, initial_draft_id: result.draft.draftId, stage: 'follow_ups' });
    } else {
      ...
      const { plan } = await (dependencies.createPlan || (await import('./campaigns-v2/plan')).createFirstContactPlan)({ ...access, client, deferGeneration: true, body: {
        ...
        steps: RESEARCH_SEQUENCE_STEPS.map((step) => ({ ...step })),
      } });
      if (plan.steps.length !== 3 || plan.autoSend || plan.lifecycleState !== 'draft') throw new Error('La secuencia cambió. Revisa su plan antes de continuar.');
      ...
      if (missing) {
        ...
        const editorial = await (dependencies.review || validateResearchSequence)(job.prepared_context.brief, drafts);
        // Cost telemetry only: the deterministic short-circuit makes no model
        // call, so there is nothing to bill. Never blocks the save below.
        if (editorial.usage) {
          await (dependencies.recordResearchAttempt || persistResearchGenerationAttempt)({
            ...
            stage: 'sequence_editorial',
            model: editorial.model,
            promptVersion: RESEARCH_SEQUENCE_EDITORIAL_PROMPT_VERSION,
            inputTokens: editorial.usage.inputTokens,
            outputTokens: editorial.usage.outputTokens,
            reasoningTokens: editorial.usage.reasoningTokens,

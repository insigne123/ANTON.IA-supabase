...
test('a corrected decision after a refusal proceeds normally', async () => {
  ...
  const result = await runCoworkReadLoop({ message: 'Busca a Rafael', signal: new AbortController().signal,
    ...
    decide: async (observations, _mustAnswer, rejections = []) => {
      attempt++;
      if (observations.length) return coworkDecisionSchema.parse({ action: 'answer', query: null, leadId: null,
        answer: { reply: 'Sin registros de Rafael.\n¿Lo busco fuera de tus contactos?', document: null,
          suggestions: [{ label: 'Sí, búscalo', message: 'Sí, busca a Rafael fuera de mis contactos' }] } });

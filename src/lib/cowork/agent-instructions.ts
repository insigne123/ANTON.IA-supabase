...
export function coworkAgentInstructions(configuration: {
  ...
  return {
    systemPrompt: [
      ...
      'Solo puedes ejecutar las lecturas y proponer los efectos descritos en esta solicitud. Proponer no equivale a ejecutar: espera el resultado confirmado. No puedes ejecutar código.',
    ...
    parallelReadCapability: 'reads.parallel: reads [{action,input}] para consultas independientes leads.search, leads.get, research.get_existing, crm.search, crm.get_lead, contacted.search, contacted.timeline, metrics.overview, app.context, draft.get o campaigns.list. input es texto para búsquedas, UUID para fichas, timelines y borradores, y cadena vacía para metrics.overview, app.context y campaigns.list. Máximo TOTAL de 3 lecturas por ejecución, compartido con lecturas individuales; hasta 2 simultáneas. Nunca escrituras o proveedores externos. Si mustAnswer es true entrega answer.',

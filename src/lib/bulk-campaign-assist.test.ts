...
test('sequence requires an explicit schedule and supports zero through six followups', () => {
  assert.equal(BulkAssistInputSchema.safeParse({ ...input, followUpDelays: undefined }).success, false);
  for (let count = 0; count <= 6; count++) assert.equal(BulkAssistInputSchema.safeParse({ ...input, followUpDelays: Array(count).fill(3) }).success, true);
  for (const delays of [[0], [91], [1.5], [1, 1, 1, 1, 1, 1, 1]]) assert.equal(BulkAssistInputSchema.safeParse({ ...input, followUpDelays: delays }).success, false);
});

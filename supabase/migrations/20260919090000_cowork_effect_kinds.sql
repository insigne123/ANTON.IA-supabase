-- Fase 2: nuevos tipos de efecto conversacional. Cada tipo se activa solo cuando
-- su ejecutor correspondiente existe en codigo; la restriccion solo amplía el
-- vocabulario permitido, no otorga permisos.
...
alter table public.cowork_effect_proposals add constraint cowork_effect_proposals_kind_check
  check (kind in ('save_contact', 'start_research', 'request_draft', 'enrich_contact',
    'send_email', 'campaign_create', 'campaign_activate', 'campaign_pause'));
...
begin
  if p_kind is null or p_kind not in ('save_contact','start_research','request_draft','enrich_contact',
    'send_email','campaign_create','campaign_activate','campaign_pause') then

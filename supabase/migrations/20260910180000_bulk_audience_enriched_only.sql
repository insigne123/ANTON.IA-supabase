-- Forward-only. Campaign audiences resolve to enriched (Apollo) leads when requested.
-- Replaces the 13-arg search signature; the old overload is dropped to avoid ambiguity.
drop function if exists public.search_bulk_audience_v1(uuid, uuid, text, text[], text[], text[], text[], text[], integer, boolean, text, integer, integer);
...
create function public.search_bulk_audience_v1(
  ...
  p_enriched_only boolean default false
...
begin
  ...
  candidates as (
    select email,
      ...
      bool_or(priority = 1) as is_enriched
  ...
  people as (
    select c.email, coalesce(c.name, '') as name, coalesce(c.company, '') as company, coalesce(c.title, '') as title,
      ...
      coalesce(c.is_enriched, false) as enriched,
  ...
  filtered as (
    select * from people p
      ...
      and (not coalesce(p_enriched_only, false) or p.enriched)
...
revoke all on function public.search_bulk_audience_v1(uuid, uuid, text, text[], text[], text[], text[], text[], integer, boolean, text, integer, integer, boolean) from public, anon, authenticated;
grant execute on function public.search_bulk_audience_v1(uuid, uuid, text, text[], text[], text[], text[], text[], integer, boolean, text, integer, integer, boolean) to service_role;

...
- `APOLLO_API_KEY` (server-only y exclusivamente en `backend-antonia`; nunca en Next ni en variables `NEXT_PUBLIC_*`)
...
Firebase Scheduled Functions es la única propietaria de los workers, campanas, reconciliaciones, observabilidad Apollo, reply sync, retencion de privacidad y rollups de ANTON.IA. Al desplegar Functions, Firebase administra los Cloud Scheduler jobs subyacentes.
...
| Uso Apollo | `apolloUsageTick` | cada hora | Captura snapshots sanitizados mediante `/api/cron/apollo-usage`. |
...
- Confirmar que `APOLLO_API_KEY` existe solo en el gateway, que `ENRICHMENT_SERVICE_SECRET` coincide entre gateway y Next, y que `INTERNAL_API_SECRET`, `UNSUBSCRIBE_TOKEN_SECRET`, `CRON_SECRET`, `LEAD_RESEARCH_WORKER_SECRET`, `FIREBASE_SCHEDULER_SECRET` y `SUPABASE_SERVICE_ROLE_KEY` existen en sus runtimes server-only
- Confirmar que `chrome-extension.pem` no viaja en el artefacto final ni en imágenes de runtime
- Verificar los nueve ticks propietarios en Cloud Scheduler y revisar sus logs de Firebase Functions
...
- Verificar callback opaco, polling y settlement idempotente de Apollo sin exponer PII ni secretos en URLs o logs

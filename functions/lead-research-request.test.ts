...
test('firebase investigation always uses the native research provider', () => {
    assert.match(indexSource, /const requestPayload = leadResearchPayload/);
    assert.match(indexSource, /const providerUrl = getLeadResearchUrl\(\)/);
    assert.match(indexSource, /'Idempotency-Key': researchRequestIdempotencyKey/);
    assert.doesNotMatch(indexSource, /LEAD_RESEARCH_USE_N8N|ANTONIA_N8N_WEBHOOK_URL|N8N_RESEARCH_WEBHOOK_URL|N8N_WEBHOOK_URL|unwrapN8nResearchResponse/);
});
